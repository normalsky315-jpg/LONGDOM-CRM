...
//  7. 複製 exec 網址 → 貼到 ltcj.html 的 GAS_URL
//  8. 到 LINE Developers Console 建立新的 LIFF App，
//     Endpoint URL 填 GitHub Pages 上 ltcj.html 的網址，
//     取得 LIFF ID → 貼到 ltcj.html 的 LIFF_ID
//  9. 上傳 ltcj.html 到 GitHub Pages
//  10.（要推播才需要）執行 setLineToken(...)／setLinePushTarget(...)／
//      setLineChannelSecret(...)，填這個案場自己 LINE 官方帳號的憑證
//  11. 執行 addUser('你的userId','你的名字','admin','龍廷誠家') 加入第一位管理員
// ============================================================
...
const CONFIG = {
  ...
  PROJECT_NAME:   '龍廷誠家',
  SPREADSHEET_ID: '請貼上龍廷誠家試算表的_SPREADSHEET_ID'
};
...
function firstTimeSetup() {
  initAllSheets();
  Logger.log('✓ 完成！龍廷誠家專用版已初始化。');
  Logger.log('下一步：');
  Logger.log('1. 若還沒設密碼，執行 setCompanyPassword(你要的密碼)');
  Logger.log('2. 部署 Web App，把 exec 網址貼到 ltcj.html 的 GAS_URL');
  Logger.log('3. 到 LINE Developers Console 建立新 LIFF App，網址貼到 ltcj.html 的 LIFF_ID');
  Logger.log('4. 執行 setLineToken(你的Token) 設定推播（需要這個案場自己的 LINE 官方帳號 Token）');
  Logger.log('5. 執行 setLinePushTarget(你的userId) 設定推播目標');
  Logger.log('6. 執行 addUser(你的userId,你的名字,admin,龍廷誠家) 加入第一位管理員');
}
...
// setLineToken('請貼上龍廷誠家的LINE_CHANNEL_ACCESS_TOKEN');
// setLinePushTarget('請貼上要接收推播的LINE_userId');
// setLineChannelSecret('請貼上龍廷誠家的LINE_CHANNEL_SECRET');

function addMyself() {
  addUser('請貼上你的LINE_userId', '請填你的名字', 'admin', '龍廷誠家');
}
