...
function syncWeeklyReportLayout_(ss) {
  ...
  reaction.getRange('A3').setValue('項目');
  ...
  reaction.getRange('A16').setValue('區域反應');
  reaction.getRange('A17').setValue('項目');
  var districtColsWithUnknown = (getAllConfigOptions_().district || []).concat([WEEKLY_REPORT_UNKNOWN_LABEL]);
  reaction.getRange(17, 2, 1, districtColsWithUnknown.length + 1).setValues([districtColsWithUnknown.concat(['合計'])]);
  ['本週來人','累計來人','本週成交','累計成交'].forEach(function(label, i) {
    reaction.getRange(18 + i, 1).setValue(label);
  });
