...
//    1. 銷控表已收訂/已保留格子點「標記成交」時，戶別/樓層/預計簽約
//       日/訂金都要重新打一次——因為 openDealModal 原本永遠用陽春
//       預設值開表單，完全沒有去查這位客戶業務端「成交階段」早就填過
//       的資料。改法：openDealModal 新增從客戶資料（sales_deal_unit_id／
//       expected_sign_date／新增的 deposit_amount）自動帶入戶別/預計
//       簽約日/訂金，不用再重打一次；業務員姓名也改成帶這位客戶實際
//       接待的業務員，不是操作當下登入的主管自己。
//    2. 型別下拉選單原本沒有 onchange，選了不同型別時樓層清單不會
//       跟著重新整理，看起來像樓層選不了——補上
//       onchange="populateDealFloorSelect()"。
//    3. 業務端「成交階段」新增 deposit_amount 欄位：選「已下訂」時
//       多一個訂金金額輸入框（選填），後端 updateCustomerDealStage
//       寫入 Customer_Data.deposit_amount（新增 ensureCustomerDepositColumn_
//       自動補欄位，Customer_Data 沒有像 Deal_Detail 一樣的自動補欄位
//       機制，第一次用到才補上，不用使用者自己去 Sheet 手動加欄）。
//       這樣主管在銷控表標記成交時，訂金也能一起自動帶入。
//  v9.41 變更：客戶卡片移除「標記成交／編輯成交／標記退戶」，成交明細
//  統一改到銷售控制表操作（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    後端新增 refundDealDetailForUnit_ 共用函式，並接進
//    updateSalesControlUnit：只要一戶從已保留/已收訂/已簽約改回退戶／
//    待售、且原本確實連結著客戶，就把該戶還「active」的成交明細標記
//    退戶、客戶資料的正式成交狀態一併改回退戶——不管退戶是從銷控表
//    「編輯戶別」直接改下拉選單，還是走完整的「標記退戶」表單，結果
//    都一致，補上之前「銷控表直接改退戶完全不會同步」的缺口（原本
//    occupiedStatuses 只處理已保留/已收訂/已簽約要連結客戶，退戶/待售
//    完全沒有對應的收尾）。
//    前端（jltx.html）配合的改動：客戶卡片拿掉三顆按鈕；銷控表已簽約
//    格子點下去可以選編輯成交明細/標記退戶（沿用既有 dealModal，只是
//    入口搬過來）；「編輯戶別」視窗把待售/已保留的戶別改成已收訂/已
//    簽約時，改成直接導去開「標記成交」表單一次填齊訂金/簽約狀態，
//    不會再有繞過去、漏填成交明細的路徑。
//  v9.40 變更：修正銷控表直接改「已簽約」時，客戶卡片「成交階段」沒
//  跟著同步的缺漏（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    根因：syncCustomerFromSalesControl_ 反向同步銷控表狀態回客戶資料
//    時，已保留／已收訂分支都會順便更新 sales_deal_stage／
//    sales_deal_unit_id／sales_deal_unit_label，但已簽約分支只更新了
//    deal_status／deal_unit，漏了成交階段欄位本身。結果：從銷控表直接
//    把某戶改成已簽約（不是走客戶列表「標記成交」）時，該客戶卡片的
//    「成交階段」仍停在原本的狀態（甚至還沒填過就是空白／未成交），
//    造成主管誤以為這個客戶還沒標記，同時因為戶別選單只顯示「待售或
//    已連結這位客戶的戶別」，這戶當然也不會出現在其他客戶的成交階段
//    選單裡——這部分選單邏輯本身是對的，問題在客戶卡片沒同步更新。
//    改法：已簽約分支比照已保留／已收訂分支，一併寫入
//    sales_deal_stage='已簽約'、sales_deal_unit_id、
//    sales_deal_unit_label，三個分支邏輯對齊。
//  v9.39 變更：週報表試算表套用主管提供、GPT 優化過的視覺設計（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    資料欄位與版面結構完全沒變，純粹加上一套配色/字體/框線/欄寬——
//    深藍當主色、中藍當區塊標題、藍灰當欄位標題、淺藍/淺灰/淺綠交替
//    當資料列、琥珀色標示合計欄。新增 WR_STYLE 統一定義顏色、wrStyle_
//    共用樣式小工具，套用到全部 5 個分頁（週報總表／客戶反應統計／
//    銷售控制表／業務統計／設定說明）。
//    順便補上銷控表格子本身依狀態上色（待售/已保留/已收訂/已簽約/
//    退戶各自不同底色，跟新增的圖例列對上）——這個功能本來就有
//    （SALES_CONTROL_STATUS_COLOR_），只是顏色改成跟新配色一致，並且
//    這次新增了圖例列本身（原本完全沒有圖例，只有格子顏色，看不出
//    顏色代表什麼）。
//  v9.38 變更：統一「已簽約＝已成交」的同步邏輯，修正主管反映銷控表
//  顯示已簽約、但客戶資料仍是未成交的不一致（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    根因：「已簽約→客戶正式成交狀態」這件事原本分散在不同入口各自
//    處理——客戶列表「標記成交」完整流程，是靠前端連續呼叫兩支 API
//    （saveDealDetail 寫成交明細+同步銷控表 → 成功後再呼叫
//    updateCustomerDeal 補寫客戶正式成交狀態），中間任一步失敗就會
//    卡在不一致狀態；首頁「待簽約提醒→標記已簽約」快速按鈕只呼叫
//    saveDealDetail，完全沒有呼叫 updateCustomerDeal 補這一步。
//    改法：saveDealDetail 只要合約狀態確定是「已簽約」，自己就直接
//    把客戶正式成交狀態同步成「已成交」，不管是哪個入口呼叫、前端
//    後續有沒有再另外呼叫 updateCustomerDeal 都一樣，統一在這支收斂，
//    保證「已簽約」一定等於「已成交」。
//  v9.37 變更：週報表客戶反應統計修正 + 銷控表反查客戶（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    1. 【週報表累計成交數字兜不起來】行業別反應／年齡反應的統計，原本
//       只要客戶資料的職業／年齡欄位是空白（兩者在客戶登記表單都不是
//       必填），或存的是「其他：ＸＸ」這種自由文字，比對邏輯就會直接
//       跳過那一筆、不算進任何欄位，導致這兩張表的「累計成交」總和比
//       實際成交數少。新增 weeklyReportBucketLabel_ 統一處理：對得到
//       清單就算那一欄，「已知欄位：自由文字」算進對應欄位，兩者都對
//       不到（含空白）才歸類「未填寫」，確保總和一定等於實際成交數。
//    2. 【新增區域反應】客戶反應統計新增第三張表（居住行政區），跟
//       行業別／年齡反應同一套邏輯與版面（週報試算表「客戶反應統計」
//       分頁新增 16~21 列）。居住行政區選項是 Config_Options 動態設定，
//       每次同步都重新抓一次目前的選項。
//    3. 【銷控表反查客戶】銷控表戶別卡片上的客戶姓名原本只是純文字，
//       改成可點擊，點下去直接跳到客戶列表並帶入姓名搜尋，不用自己
//       手動去客戶名單找。
//  v9.36 變更：速度與連線穩定性優化（詳見
//  docs/superpowers/specs/2026-09-19-jltx-speed-stability-design.md）：
//    1. 【防止逾時重試造成重複寫入】appendCustomerData 在 v9.11 已有
//       client_request_id + CacheService 的「認領＋輪詢」防護，但
//       saveDealDetail（記錄成交明細）／appendContactLog（新增追蹤
//       記錄）／updateCustomerData（編輯客戶資料）這三支一樣會同步
//       呼叫 Supabase 雙寫、一樣可能被前端逾時重試，卻完全沒有這層
//       防護。這次比照同樣的機制補上（saveDealDetail／
//       appendContactLog 防止真的多寫一筆；updateCustomerData 防止
//       CHANGE_LOG 多出重複的修改紀錄、Supabase 被多 PATCH 一次）。
//    2. 【首頁登入後打太多 API】新增 getHomeBootstrap，把登入後
//       loadGlobalData／renderHome 原本各自獨立呼叫的
//       getProjectList／getIndustryList／getPurchaseMotiveList／
//       getConfigOptions／getTasks／getCustomerList／getTodayLeave
//       （主管以上再加 getUserList）合併成一次回傳，前端從原本 7-8
//       個 API 請求減少到 2 個（登入時 1 個、首頁渲染 1 個）。內部
//       直接呼叫既有函式，不重寫任何邏輯；單一子項目失敗不影響其他
//       子項目。
//    3. 【回籠客人連結自動帶入】searchMyCustomers 的搜尋結果補上
//       age／district／detailed_address／occupation_industry／
//       purchase_motive／source／gender／marital_status，供
//       jltx.html 選擇連結客人時自動帶入表單基本資料，不用重打一次
//       （見 pickLinkCustomer）。
//  v9.35 變更：修正一批「銷控表／客戶資料兩邊對不起來」的資料一致性
//  漏洞，加上週報表試算表的幾個顯示錯誤：
//    1. 【銷控表直接編輯戶別】原本可以不指定客戶就把狀態改成已保留/
//       已收訂/已簽約（真實案例：A2/5F 在銷控表勾了已簽約，客戶資料
//       完全沒有跟著變已成交）。updateSalesControlUnit()／
//       appendSalesControlUnit() 現在改成已保留/已收訂/已簽約一定要
//       指定連結的客戶（新增 syncCustomerFromSalesControl_() 反向同步
//       回客戶資料的成交階段/成交狀態），狀態退回待售/退戶時順便清掉
//       舊的客戶連結。jltx.html 銷控表編輯視窗新增「連結客戶」欄位
//       （狀態選已保留/已收訂/已簽約時才顯示，必填）。
//    2. 【標記成交／成交明細】saveDealDetail() 原本允許 customer_id
//       空白也能送出成功，建出來的成交明細查無此人。改成一定要對到
//       既有客戶資料，前端 confirmDeal() 也加了同樣的擋下（提示改從
//       清單挑選）。
//    3. 【客戶卡片 14 天限制沒判斷角色】myCustomerCardHTML() 的
//       ✏️修改／🗑️刪除按鈕原本只看建立天數，沒有排除主管/admin——
//       後端 updateCustomerData/deleteCustomerData 其實只限制業務，
//       主管本來就能隨時改，但畫面上超過 14 天一律隱藏按鈕，導致主管
//       誤以為系統擋住無法修改。改成跟後端規則一致（!isSales() ||
//       diffDays<=14）。同時「我的客戶」清單原本完全沒有標記成交／
//       編輯成交／標記退戶這幾顆主管專用按鈕（只有業務自填的「成交
//       階段」，設計上就不包含已成交），現在補上，跟「近期客戶」清單
//       一樣的權限規則。
//    4. 【週報表 Excel：客戶反應統計欄位跟資料對不起來】主管反映欄位
//       標題跟底下數字錯開一欄（例如「醫務」欄底下顯示的其實是「上班
//       族」的數字）——因為標題從 A 欄開始放類別名稱，數字卻從 B 欄
//       開始寫。改成標題也從 B 欄開始，A 欄固定放「項目」列標籤。
//    5. 【週報表 Excel：職業別分類跟系統實際選項不一致】原本把系統
//       實際 21 個職業選項用一張表併成範本的 14 類（例如公教/軍人/
//       警察都算「軍公教」），但案場明確要求這三個要分開統計。改成
//       直接用 CONFIG.INDUSTRIES 當欄位，不做任何合併，選項改了這裡
//       也會自動跟著變。
//    6. 【週報表 Excel：銷控表分頁完全空白】即使 Sales_Control 裡有
//       戶別已經是已簽約，銷控表分頁還是整張空白（連「待售」都不會
//       顯示）。根因是 building/unit_type 從試算表讀出來可能是數字
//       型別，拿來當物件 key 用時被 JS 自動轉成字串，但後面比對是不
//       是同一戶用嚴格比較（===），數字 1 永遠不等於字串 "1"，導致
//       每一戶都比對不到。統一用 String() 包住兩邊再比較。
//    7. 【週報表 Excel：欄位改版不會套用到已存在的試算表】上面第 4/5
//       點的版面修正，如果只在「第一次建立試算表」時套用，已經建立
//       過的舊試算表不會自動修正。改成每次同步都重新確認一次版面
//       （ensureWeeklyReportSpreadsheet_()／syncWeeklyReportLayout_()，
//       分頁用 getSheetByName 找得到就沿用、找不到才新增），舊試算表
//       下次同步就會自動修正成最新版面。
//    8. 【業務統計：接客合計/回訪率算法沒有意義】原本 total_visit／
//       total_revisit 是把初訪＋回籠的「來訪列數」直接加總，同一人
//       回籠愈多次，回訪率的分子分母會一起膨脹，例如接待 3 組客人、
//       其中 1 組回籠一次，用列數算出來的回訪率不會是想要的 1/3。
//       改成先依人（person_id，沒有的話退回 customer_id）分組，
//       total_visit/total_revisit/total_deal 改成「人數」而不是
//       「列數」，回訪率＝回訪過的人數／總人數。
...
// ★ 效能優化：上面提到「首頁一次載入平行發出 7、8 個 API 請求」，
// getUserContext 的 60 秒快取只省掉重讀 User_Role_Table 的時間，
// 每一個請求還是各自一趟完整的 HTTPS 來回，這才是主要的延遲來源。
// 這支把首頁登入後會用到的資料一次彙整回傳，前端只打一次 API。
// 內部直接呼叫既有函式，不重寫任何邏輯；單一子項目失敗（回傳
// ok:false）不影響其他子項目，前端各自沿用原本的失敗處理方式。
function getHomeBootstrap(payload) {
  var lineUserId = payload && payload.lineUserId;
  var ctx = getUserContext(lineUserId);
  if (!ctx) return fail('未授權');

  function safe(fn) {
    try { return fn(); } catch (err) { return fail(err.message); }
  }

  var data = {
    projects:      safe(function(){ return getProjectList(); }),
    industries:    safe(function(){ return getIndustryList(); }),
    motives:       safe(function(){ return getPurchaseMotiveList(); }),
    configOptions: safe(function(){ return getConfigOptions(); }),
    tasks:         safe(function(){ return getTasks({ lineUserId: lineUserId, status: 'pending' }); }),
    customers:     safe(function(){ return getCustomerList({ lineUserId: lineUserId }); }),
    todayLeave:    safe(function(){ return getTodayLeave(lineUserId); })
  };
  if (ctx.role !== CONFIG.ROLES.SALES) {
    data.userList = safe(function(){ return getUserList({ lineUserId: lineUserId }); });
  }
  return ok(data);
}
  ...
  try {
    switch (action) {
      case 'getHomeBootstrap':
        return jsonResponse(getHomeBootstrap(payload.lineUserId ? payload : { lineUserId: e.parameter.lineUserId }));
      case 'getProjectList':
...
function saveDealDetail(payload) {
  try {
    ...
    // 防止重複建檔：跟 appendCustomerData 同樣的成因——這支 API 會寫
    // Sheets + 同步 Supabase，耗時常常超過前端 gasFetch 的 10 秒逾時，
    // 逾時後前端自動重試一次（同一個 client_request_id）。新建成交明細
    // 時 deal_id 是重試那次才生的（existing 判斷不到還沒寫進表的那筆），
    // 兩次呼叫會各自 genId 出不同 deal_id、變成兩筆。做法比照
    // appendCustomerData：先認領（存 PROCESSING）再處理，重試那次看到
    // PROCESSING 就輪詢等最終結果，不會自己重新跑一次。
    var idemKey = payload.client_request_id ? 'dealdetail_' + payload.client_request_id : null;
    var idemCache = idemKey ? CacheService.getScriptCache() : null;
    if (idemKey) {
      var existingIdem = idemCache.get(idemKey);
      if (existingIdem && existingIdem !== 'PROCESSING') return JSON.parse(existingIdem);
      if (existingIdem === 'PROCESSING') {
        for (var waitMs = 0; waitMs < 15000; waitMs += 500) {
          Utilities.sleep(500);
          var polled = idemCache.get(idemKey);
          if (polled && polled !== 'PROCESSING') return JSON.parse(polled);
        }
        return fail('前一筆送出仍處理中，請稍後查看成交明細確認是否已建立，避免重複建檔');
      }
      idemCache.put(idemKey, 'PROCESSING', 120);
    }
    ...
    // ★ 鎖死：customer_id 一定要能對到既有客戶資料，不能讓「客戶」欄位
    // 打了名字但選不到既有客戶時，還是用空白 customer_id 送出成功——
    // 這樣建出來的成交明細跟銷控表連結的戶別，查無此人，跟客戶資料完全
    // 兜不起來。前端要嘛從既有客戶名單挑，要嘛先去客戶資料建檔
    var dealCustomerId = payload.customer_id || (existing && existing.customer_id) || '';
    var dealCustomer = getCustomerRowById_(dealCustomerId);
    if (!dealCustomer) return fail('請從既有客戶名單挑選客戶，找不到的話請先到客戶資料建檔');
    ...
    var row = {
      ...
      customer_name:        dealCustomer.customer_name,
    ...
    // ★ v9.38 修正：「已簽約＝已成交」這條規則原本分散在不同入口各自
    // 處理——客戶列表「標記成交」的完整流程會在這支成功後，由前端另外
    // 呼叫一次 updateCustomerDeal 補寫客戶正式成交狀態；但首頁「待簽約
    // 提醒→標記已簽約」這個快速按鈕只呼叫這支，完全沒有補寫那一步，
    // 導致銷控表/成交明細都顯示已簽約，客戶資料卻還停在未成交。
    // 改成只要合約狀態是「已簽約」，這支自己就順手把客戶正式成交狀態
    // 同步好，不管是哪個入口呼叫、不管前端後續有沒有再另外呼叫
    // updateCustomerDeal，都保證已簽約一定等於已成交，不會再漏同步。
    // updateRowById 對同一列重複寫一樣的值是安全的操作，即使前端仍舊
    // 呼叫 updateCustomerDeal 也不會有副作用。
    if (contractStatus === '已簽約') {
      updateRowById(CONFIG.SHEETS.CUSTOMER, 'customer_id', row.customer_id, {
        deal_status: '已成交',
        deal_unit: row.unit || dealCustomer.deal_unit || '',
        updated_at: nowTW()
      });
    }

    var result = ok(row);
    if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(result), 120); } catch (e) {} }
    return result;
  } catch (err) {
    Logger.log('saveDealDetail error: ' + err);
    var failResult = fail(err.message);
    if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(failResult), 120); } catch (e2) {} }
    return failResult;
  }
...
function appendContactLog(payload) {
  try {
    ...
    // 防止重複建檔：比照 appendCustomerData／saveDealDetail，這支也會
    // 同步呼叫 Supabase 雙寫，可能超過前端 10 秒逾時被自動重試一次
    var idemKey = payload.client_request_id ? 'contactlog_' + payload.client_request_id : null;
    var idemCache = idemKey ? CacheService.getScriptCache() : null;
    if (idemKey) {
      var existingIdem = idemCache.get(idemKey);
      if (existingIdem && existingIdem !== 'PROCESSING') return JSON.parse(existingIdem);
      if (existingIdem === 'PROCESSING') {
        ...
        return fail('前一筆送出仍處理中，請稍後查看追蹤記錄確認是否已建立，避免重複建檔');
      }
      idemCache.put(idemKey, 'PROCESSING', 120);
    }
    ...
    Logger.log('appendContactLog error: ' + err);
    var failResult = fail(err.message);
    if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(failResult), 120); } catch (e2) {} }
    return failResult;
  }
...
function searchMyCustomers(payload) {
  try {
    ...
    // 除了搜尋結果顯示用的欄位，也補上基本資料欄位，讓前端在業務挑選
    // 「回籠客人連結」時可以直接自動帶入表單，不用使用者重打一次
    // （見 jltx.html pickLinkCustomer）
    var results = rows.slice(0, 15).map(function(r) {
      return {
        ...
        purchase_motive:     r.purchase_motive || '',
        source:              r.source || '',
        gender:              r.gender || '',
...
function updateCustomerData(payload) {
  try {
    ...
    // 防止重複寫入：比照 appendCustomerData 等其他寫入 API。這支雖然是
    // updateRowById（同一個欄位被寫兩次結果不會壞），但沒有這層防護的
    // 話，逾時自動重試會讓 CHANGE_LOG 多出一筆一模一樣的「重複修改」
    // 紀錄、Supabase 也會被多呼叫一次 PATCH，所以一樣加上防護
    var idemKey = payload.client_request_id ? 'custupdate_' + payload.client_request_id : null;
    var idemCache = idemKey ? CacheService.getScriptCache() : null;
    if (idemKey) {
      var existingIdem = idemCache.get(idemKey);
      if (existingIdem && existingIdem !== 'PROCESSING') return JSON.parse(existingIdem);
      if (existingIdem === 'PROCESSING') {
        ...
        return fail('前一筆送出仍處理中，請稍後重新整理確認是否已更新');
      }
      idemCache.put(idemKey, 'PROCESSING', 120);
    }
    ...
    if (!changes.length) {
      var noChangeResult = ok({ customer_id: payload.customer_id, message: '無變更' });
      if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(noChangeResult), 120); } catch (e) {} }
    ...
    var result = ok({ customer_id: payload.customer_id, changes_count: changes.length });
    if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(result), 120); } catch (e) {} }
    return result;
  } catch (err) {
    Logger.log('updateCustomerData error: ' + err);
    var failResult = fail(err.message);
    if (idemKey) { try { CacheService.getScriptCache().put(idemKey, JSON.stringify(failResult), 120); } catch (e2) {} }
    return failResult;
  }
...
// Customer_Data 沒有像 Deal_Detail（ensureDealDetailSheet）那樣自動補
// 欄位的機制，欄位都是當初手動建表時就固定好的。這次要新增
// deposit_amount（業務填成交階段=已下訂時可以順便填訂金），比照同樣的
// 「用到才自動補欄位」做法，第一次用到時自動幫忙在 Customer_Data 補上
// 這個欄位，不需要使用者自己手動去 Google Sheet 加欄位（避免又一次
// 部署漏一步）
function ensureCustomerDepositColumn_() {
  var sh = getSheet(CONFIG.SHEETS.CUSTOMER);
  if (!sh) return;
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
  if (headers.indexOf('deposit_amount') >= 0) return;
  sh.getRange(1, headers.length + 1).setValue('deposit_amount');
}
...
function updateCustomerDealStage(payload) {
  try {
    ...
    var depositAmount = stage === '已下訂' ? (payload.deposit_amount || '') : '';

    ensureCustomerDepositColumn_();
...
function getCustomerRowById_(customerId) {
  if (!customerId) return null;
  var rows = readSheetAsObjects(CONFIG.SHEETS.CUSTOMER);
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i].customer_id) === String(customerId)) return rows[i];
  }
  return null;
}

// ★ 銷控表直接編輯戶別狀態改成已保留／已收訂／已簽約時，反向同步回
// 客戶資料。saveDealDetail()/syncSalesControlFromDeal_() 是「客戶資料→
// 銷控表」的正向同步，這裡補上反方向，不然從銷控表直接改狀態就會出現
// 「銷控表顯示已簽約，客戶資料卻還是未成交」的資料對不起來問題（這正是
// 這次修正的起因：A2/5F 在銷控表勾了已簽約，客戶資料那邊完全沒有跟著變）
function syncCustomerFromSalesControl_(customerId, status, unit, reservedUntil, expectedSignDate) {
  if (!customerId) return;
  var updates = { updated_at: nowTW() };
  if (status === '已保留') {
    updates.sales_deal_stage = '已保留';
    updates.sales_deal_unit_id = unit.unit_id;
    updates.sales_deal_unit_label = unit.unit_label;
    updates.reserved_until = reservedUntil || '';
    updates.expected_sign_date = '';
  } else if (status === '已收訂') {
    updates.sales_deal_stage = '已下訂';
    updates.sales_deal_unit_id = unit.unit_id;
    updates.sales_deal_unit_label = unit.unit_label;
    updates.expected_sign_date = expectedSignDate || '';
    updates.reserved_until = '';
  } else if (status === '已簽約') {
    updates.sales_deal_stage = '已簽約';
    updates.sales_deal_unit_id = unit.unit_id;
    updates.sales_deal_unit_label = unit.unit_label;
    updates.deal_status = '已成交';
    updates.deal_unit = unit.unit_label;
  }
  updateRowById(CONFIG.SHEETS.CUSTOMER, 'customer_id', customerId, updates);
}

// 銷控表把某戶從已保留/已收訂/已簽約改回退戶（或直接改回待售），且這戶
// 原本確實連結著客戶——代表這筆交易告吹了。跟 markDealDetailRefund／
// updateCustomerDeal（客戶卡片以前「標記退戶」用的兩支 API）做同一件
// 事：把這戶還「active」的成交明細標記退戶、客戶資料的正式成交狀態也
// 改回退戶，確保不管退戶是從銷控表直接改狀態、還是走完整的標記退戶
// 表單，結果都一致，不會有一邊退戶了、另一邊還顯示已成交
function refundDealDetailForUnit_(unitId, customerId, reason, ctx) {
  if (!unitId) return;
  ensureDealDetailSheet();
  var deals = readSheetAsObjects(CONFIG.SHEETS.DEAL_DETAIL).filter(function(r) {
    return String(r.unit_id) === String(unitId) && r.status === 'active';
  });
  deals.forEach(function(d) {
    updateRowById(CONFIG.SHEETS.DEAL_DETAIL, 'deal_id', d.deal_id, {
      status: '退戶', refund_reason: reason || '', refund_date: todayTW(), updated_at: nowTW()
    });
  });
  if (customerId) {
    updateRowById(CONFIG.SHEETS.CUSTOMER, 'customer_id', customerId, {
      deal_status: '退戶', updated_at: nowTW()
    });
  }
  if (ctx) {
    writeAuditLog(ctx.lineUserId, 'UPDATE', CONFIG.SHEETS.SALES_CONTROL, unitId,
      ctx.displayName + ' 從銷控表標記退戶：' + unitId);
  }
}
...
function appendSalesControlUnit(payload) {
  try {
    ...
    // ★ 鎖死：新增戶別時如果直接選了已保留/已收訂/已簽約，一樣要指定
    // 客戶，理由跟 updateSalesControlUnit 一樣
    var linkedCustomer = null;
    if (['已保留','已收訂','已簽約'].indexOf(status) >= 0) {
      linkedCustomer = getCustomerRowById_(payload.linked_customer_id);
      if (!linkedCustomer) return fail('狀態選「' + status + '」時，必須指定連結的客戶');
    }
    ...
    var row = {
      ...
      linked_customer_id: linkedCustomer ? linkedCustomer.customer_id : '',
      linked_customer_name: linkedCustomer ? linkedCustomer.customer_name : '',
    ...
    if (linkedCustomer) {
      syncCustomerFromSalesControl_(linkedCustomer.customer_id, status,
        { unit_id: unitId, unit_label: row.unit_label }, row.reserved_until, row.expected_sign_date);
    }
    writeAuditLog(ctx.lineUserId, 'CREATE', CONFIG.SHEETS.SALES_CONTROL, unitId,
      ctx.displayName + ' 新增銷控戶別: ' + row.unit_label + (linkedCustomer ? '，客戶：' + linkedCustomer.customer_name : ''));
...
function updateSalesControlUnit(payload) {
  try {
    ...
    // ★ 鎖死：狀態要改成「已保留／已收訂／已簽約」代表這戶已經跟某位
    // 客戶談到某個階段，一定要指定是哪位客戶，不能讓戶別憑空變成有人談
    // 卻查無此人。這是修正「銷控表直接改狀態可以不填客戶，導致客戶資料
    // 完全不會同步」的漏洞——沒填就直接擋下來，不讓這筆更動送出
    var occupiedStatuses = ['已保留','已收訂','已簽約'];
    var linkedCustomer = null;
    if (occupiedStatuses.indexOf(status) >= 0) {
      var linkedCustomerId = payload.linked_customer_id || original.linked_customer_id || '';
      linkedCustomer = getCustomerRowById_(linkedCustomerId);
      if (!linkedCustomer) return fail('狀態選「' + status + '」時，必須指定連結的客戶');
    }
    ...
    // 狀態退回「待售／退戶」時，順便清掉舊的客戶連結——這戶已經跟這個
    // 客戶沒關係了，留著舊的 linked_customer_id 只會讓下次查詢誤以為
    // 還連著人
    var updates = {
      status: status,
      reserved_until: reservedUntil,
    ...
    // 從已保留/已收訂/已簽約改回退戶/待售，且這戶原本連結著客戶——
    // 這筆交易告吹了，收尾把成交明細跟客戶資料都同步成退戶，不管是
    // 直接改這裡的下拉選單、還是走「標記退戶」表單，結果都一致
    var wasOccupied = occupiedStatuses.indexOf(original.status) >= 0 && original.linked_customer_id;
    var stillOccupied = occupiedStatuses.indexOf(status) >= 0;
    if (wasOccupied && !stillOccupied) {
      refundDealDetailForUnit_(payload.unit_id, original.linked_customer_id, '', ctx);
    }

    if (linkedCustomer) {
      syncCustomerFromSalesControl_(linkedCustomer.customer_id, status,
        { unit_id: payload.unit_id, unit_label: original.unit_label }, reservedUntil, expectedSignDate);
    }
    ...
    writeAuditLog(ctx.lineUserId, 'UPDATE', CONFIG.SHEETS.SALES_CONTROL, payload.unit_id,
      ...
      (linkedCustomer ? '，客戶：' + linkedCustomer.customer_name : ''));
...
// 調整分類只要改這裡：
// ★ v9.35 修正：原本把系統實際 21 個職業選項用一張對照表併成範本的
// 14 個欄位（例如公教/軍人/警察都併成「軍公教」），主管反映這樣不對——
// 案場明確要求軍人/警察/公教要分開統計，「我上面怎麼填的，報表就該
// 照樣統計出來」。改成直接用 CONFIG.INDUSTRIES（客戶登記表單實際的
...
// ★ v9.36 修正：行業別反應／年齡反應的「累計成交」數字比實際成交筆數
// 少——根因是職業/年齡在客戶登記表單都不是必填欄位，標記成交也不會
// 檢查這兩欄有沒有填，只要欄位空白，原本的比對邏輯（完全比對清單裡的
// 固定選項）就會直接跳過這筆、不算進任何一欄，總和因此比實際成交數
// 少。「其他：ＸＸ」這種自由文字也是同樣問題，存的值對不到清單裡的
// 「其他」兩個字，一樣會被跳過。
// 這支統一處理：對得到清單裡的值就算那一欄；存的是「已知欄位：自由
// 文字」（例如「其他：接案設計師」「外縣市：台北市」）算進對應的已知
// 欄位；兩種都對不到（含完全空白）才歸類到「未填寫」，確保每一筆成交
// 一定會被算進某一欄，總和永遠對得上實際成交數，不會再無聲消失。
function weeklyReportBucketLabel_(rawValue, knownCols) {
  var val = String(rawValue || '').trim();
  if (!val) return WEEKLY_REPORT_UNKNOWN_LABEL;
  if (knownCols.indexOf(val) >= 0) return val;
  for (var i = 0; i < knownCols.length; i++) {
    if (val.indexOf(knownCols[i] + '：') === 0) return knownCols[i];
  }
  return WEEKLY_REPORT_UNKNOWN_LABEL;
}
...
function ensureWeeklyReportSpreadsheet_() {
  ...
  var ss = null;
  if (id) {
    try { ss = SpreadsheetApp.openById(id); } catch (e) { /* 檔案被刪了，往下重建 */ }
  }
  if (!ss) {
    ss = SpreadsheetApp.create(CONFIG.PROJECT_NAME + '週報表');
    setProp(CONFIG.PROP_KEYS.WEEKLY_REPORT_SHEET_ID, ss.getId());
  }
  // ★ v9.35 修正：原本版面只在第一次建立試算表時畫一次，之後改了欄位
  // 定義（例如這次把職業別從 14 類改回真正的 21 個選項）不會反映到已經
  // 存在的舊試算表，只會拿新的資料去套舊的欄位標題，造成標題跟數字對
  // 不起來。改成每次同步都重新確認一次版面（getSheetByName 找得到就
  // 沿用分頁，找不到才新增；標題儲存格每次都重新蓋一次值，這是安全的
  // 重複操作），舊試算表下次同步就會自動修正成最新版面
  syncWeeklyReportLayout_(ss);
  return ss;
}

// 確保 5 個分頁存在，並且固定的標題/標籤儲存格是最新版面。這個函式
// 設計成可以重複執行（不管是第一次建立、還是舊試算表要修正版面），
// 數字本身由 writeWeeklyReportToSheet_() 另外填入
// ============================================================
// 週報表視覺設計（v9.39，依主管提供、GPT 優化過的配色版本實作）：
// 深藍當主色、中藍當區塊標題、藍灰當欄位標題、淺藍/淺灰/淺綠交替當
// 資料列、琥珀色標示合計欄。wrStyle_ 是共用的儲存格樣式小工具，顏色
// 統一定義在 WR_STYLE，之後要調色只要改這裡，不用到處找散落的色碼。
// ============================================================
var WR_STYLE = {
  titleBg: '#17365D', titleFg: '#FFFFFF',
  sectionBg: '#2F75B5', sectionFg: '#FFFFFF',
  colHeadBg: '#5B6573', colHeadFg: '#FFFFFF',
  totalBg: '#FCE4B2', totalFg: '#9C6500',
  weekRowBg: '#EEF5FB', cumRowBg: '#F3F5F7', dealRowBg: '#EDF7ED',
  valueFg: '#17365D', dataFg: '#25313C',
  statBigBg: '#D9EAF7', grandTotalBg: '#D9E1E8',
  zebraA: '#FFFFFF', zebraB: '#F3F5F7',
  effBg: '#FFF8E8', totalColBg: '#E2EAF3'
};

function wrStyle_(range, opts) {
  if (opts.bg !== undefined) range.setBackground(opts.bg);
  if (opts.fg) range.setFontColor(opts.fg);
  if (opts.bold != null) range.setFontWeight(opts.bold ? 'bold' : 'normal');
  ...
  if (opts.border) range.setBorder(null, null, true, null, null, null, '#CCCCCC', SpreadsheetApp.BorderStyle.SOLID);
}

function syncWeeklyReportLayout_(ss) {
  function sheet_(name) {
    var existing = ss.getSheetByName(name);
    if (existing) return existing;
    // 全新試算表只有一張預設的空白分頁（Sheet1），第一次同步時直接把
    // 它改名沿用，不留一張永遠用不到的空白分頁
    var sheets = ss.getSheets();
    if (sheets.length === 1) { sheets[0].setName(name); return sheets[0]; }
    return ss.insertSheet(name);
  }

  var summary = sheet_('週報總表');
  summary.getRange('A1').setValue(CONFIG.PROJECT_NAME + '｜銷售週報');
  summary.getRange('A1:K1').merge();
  wrStyle_(summary.getRange('A1:K1'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 18 });
  ...
  wrStyle_(summary.getRange('A3'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center', border: true });
  wrStyle_(summary.getRange('C3'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center', border: true });
  wrStyle_(summary.getRange('E3'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center', border: true });
  wrStyle_(summary.getRange('G3'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center', border: true });
  wrStyle_(summary.getRange('B3'), { bg: WR_STYLE.weekRowBg, fg: WR_STYLE.dataFg, bold: true, size: 10, border: true });
  wrStyle_(summary.getRange('D3'), { bg: WR_STYLE.weekRowBg, fg: WR_STYLE.dataFg, bold: true, size: 10, border: true, numFmt: 'yyyy/mm/dd' });
  wrStyle_(summary.getRange('F3'), { bg: WR_STYLE.weekRowBg, fg: WR_STYLE.dataFg, bold: true, size: 10, border: true, numFmt: 'yyyy/mm/dd' });
  summary.getRange('H3:K3').merge();
  wrStyle_(summary.getRange('H3:K3'), { bg: WR_STYLE.weekRowBg, fg: WR_STYLE.dataFg, bold: true, size: 10, border: true, align: 'left' });

  [['A4:B4','A5:B5'], ['C4:D4','C5:D5'], ['E4:F4','E5:F5'], ['G4:H4','G5:H5'], ['I4:K4','I5:K5']].forEach(function(pair) {
    summary.getRange(pair[0]).merge();
    summary.getRange(pair[1]).merge();
    wrStyle_(summary.getRange(pair[0]), { bg: WR_STYLE.colHeadBg, fg: WR_STYLE.colHeadFg, bold: true, size: 10, align: 'center', border: true });
    wrStyle_(summary.getRange(pair[1]), { bg: WR_STYLE.statBigBg, fg: WR_STYLE.valueFg, bold: true, size: 16, align: 'center', border: true });
  });
  summary.getRange('A4').setValue('本週來人');
  summary.getRange('C4').setValue('本週預訂');
  summary.getRange('E4').setValue('預訂車位');
  summary.getRange('G4').setValue('本週成交');
  summary.getRange('I4').setValue('累計成交');

  summary.getRange('A6').setValue('每日媒體安排／來人／預訂／銷售紀錄');
  summary.getRange('A6:K6').merge();
  wrStyle_(summary.getRange('A6:K6'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 11, align: 'left' });

  summary.getRange('A7:L7').setValues([['日期','星期','媒體安排','','','','','來人','預訂戶數','預訂車位','銷售紀錄／客戶異動','']]);
  summary.getRange('C7:G7').merge();
  wrStyle_(summary.getRange('A7:L7'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 10, align: 'center', border: true });
  for (var i = 0; i < 7; i++) {
    var r = 8 + i;
    summary.getRange(r, 1).setValue('');
    summary.getRange('C' + r + ':G' + r).merge();
    var zebra = i % 2 === 0 ? WR_STYLE.zebraA : WR_STYLE.zebraB;
    wrStyle_(summary.getRange(r, 1, 1, 11), { bg: zebra, fg: WR_STYLE.dataFg, align: 'center', border: true });
    wrStyle_(summary.getRange(r, 1), { numFmt: 'yyyy/mm/dd' });
  }
  summary.getRange(15, 1).setValue('總計');
  summary.getRange('C15:G15').merge();
  wrStyle_(summary.getRange(15, 1, 1, 11), { bg: WR_STYLE.grandTotalBg, fg: WR_STYLE.valueFg, bold: true, size: 10, align: 'center', border: true });

  summary.setColumnWidths(1, 2, 90); summary.setColumnWidths(3, 5, 75);
  summary.setColumnWidth(8, 60); summary.setColumnWidths(9, 2, 68); summary.setColumnWidth(11, 230);

  // ★ v9.35 修正：欄位標題原本從 A 欄開始放職業別名稱，但下面填數字時
  // 是從 B 欄開始寫（A 欄留給「本週來人」這種列標籤），造成標題永遠比
  // 底下的數字錯開一欄——例如「醫務」欄底下顯示的其實是「上班族」的
  // 數字。改成標題也從 B 欄開始放，跟數字對齊
  var reaction = sheet_('客戶反應統計');
  reaction.getRange('A1').setValue(CONFIG.PROJECT_NAME + '｜客戶反應統計');
  reaction.getRange('A1:X1').merge();
  wrStyle_(reaction.getRange('A1:X1'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 18 });

  // ★ v9.36：欄位加上「未填寫」，讓成交統計的總和一定對得上實際成交數
  // （見 weeklyReportBucketLabel_ 上方註解）
  ...
  // 現抓一次，欄位數會跟著 Config_Options 的設定變動
  ...
  var control = sheet_('銷售控制表');
  control.getRange('A1').setValue(CONFIG.PROJECT_NAME + '｜銷售控制表');
  control.getRange('A1:L1').merge();
  wrStyle_(control.getRange('A1:L1'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 18 });
  // 棟別×樓層網格內容（含圖例、棟別欄位標題）完全由 writeWeeklyReportToSheet_()
  // 依 Sales_Control 目前實際的棟別/樓層/戶型動態產生，這裡不預先畫死版面

  var salesSheet = sheet_('業務統計');
  salesSheet.getRange('A1').setValue(CONFIG.PROJECT_NAME + '｜業務成交比統計');
  salesSheet.getRange('A1:F1').merge();
  wrStyle_(salesSheet.getRange('A1:F1'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 18 });
  salesSheet.getRange('A3:B3').setValues([['期間','指標']]);
  wrStyle_(salesSheet.getRange('A3:B3'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10 });
  ['本週來客','本週回訪','本週成交','接客合計','回訪合計','成交合計','回訪率','成交比'].forEach(function(label, i) {
    salesSheet.getRange(4 + i, 2).setValue(label);
  });
  salesSheet.getRange('A4:A6').merge();
  wrStyle_(salesSheet.getRange('A4:A6'), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center' });
  wrStyle_(salesSheet.getRange('B4:B6'), { bg: WR_STYLE.weekRowBg, fg: WR_STYLE.valueFg, bold: true, size: 9 });
  salesSheet.getRange('A7:A9').merge();
  wrStyle_(salesSheet.getRange('A7:A9'), { bg: WR_STYLE.colHeadBg, fg: WR_STYLE.colHeadFg, bold: true, size: 10, align: 'center' });
  wrStyle_(salesSheet.getRange('B7:B9'), { bg: WR_STYLE.cumRowBg, fg: WR_STYLE.valueFg, bold: true, size: 9 });
  salesSheet.getRange('A10:A11').merge();
  wrStyle_(salesSheet.getRange('A10:A11'), { bg: WR_STYLE.totalBg, fg: WR_STYLE.totalFg, bold: true, size: 10, align: 'center' });
  wrStyle_(salesSheet.getRange('B10:B11'), { bg: WR_STYLE.effBg, fg: WR_STYLE.valueFg, bold: true, size: 9 });
  salesSheet.setColumnWidth(1, 60); salesSheet.setColumnWidth(2, 90); salesSheet.setColumnWidths(3, 4, 95);

  var settings = sheet_('設定說明');
  settings.getRange('A1:D1').setValues([['項目', CONFIG.PROJECT_NAME + '設定', '資料來源／依據', '備註']]);
  wrStyle_(settings.getRange('A1:D1'), { bg: WR_STYLE.titleBg, fg: WR_STYLE.titleFg, bold: true, size: 10 });
  settings.setColumnWidth(1, 100); settings.setColumnWidth(2, 260); settings.setColumnWidth(3, 160); settings.setColumnWidth(4, 260);

  ss.setActiveSheet(summary);
}

// 客戶反應統計的行業別/年齡/區域三張表版面完全一樣（項目列＋合計欄＋
// 四列本週/累計的來人/成交），抽成共用函式，styleStartRow 是每張表的
// 起始列（區塊標題那一列）
function wrStyleReactionBlock_(reaction, styleStartRow, title, colsWithUnknown) {
  var titleRow = styleStartRow, headRow = styleStartRow + 1;
  reaction.getRange(titleRow, 1).setValue(title);
  var lastCol = colsWithUnknown.length + 2; // +1 為「項目」欄、+1 為「合計」欄
  reaction.getRange(titleRow, 1, 1, lastCol).merge();
  wrStyle_(reaction.getRange(titleRow, 1, 1, lastCol), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 11, align: 'left' });

  reaction.getRange(headRow, 1).setValue('項目');
  reaction.getRange(headRow, 2, 1, colsWithUnknown.length + 1).setValues([colsWithUnknown.concat(['合計'])]);
  wrStyle_(reaction.getRange(headRow, 1, 1, colsWithUnknown.length + 1), { bg: WR_STYLE.colHeadBg, fg: WR_STYLE.colHeadFg, bold: true, size: 9, align: 'center' });
  wrStyle_(reaction.getRange(headRow, lastCol), { bg: WR_STYLE.totalBg, fg: WR_STYLE.totalFg, bold: true, size: 9, align: 'center' });

  var rowStyles = [WR_STYLE.weekRowBg, WR_STYLE.cumRowBg, WR_STYLE.dealRowBg, WR_STYLE.dealRowBg];
  ['本週來人','累計來人','本週成交','累計成交'].forEach(function(label, i) {
    var r = headRow + 1 + i;
    reaction.getRange(r, 1).setValue(label);
    wrStyle_(reaction.getRange(r, 1, 1, lastCol - 1), { bg: rowStyles[i], fg: WR_STYLE.valueFg, bold: false, size: 9, align: 'center' });
    wrStyle_(reaction.getRange(r, 1), { bold: true });
    wrStyle_(reaction.getRange(r, lastCol), { bg: WR_STYLE.totalBg, fg: WR_STYLE.totalFg, bold: true, size: 9, align: 'center' });
  });
}
...
function gatherWeeklyReportData_(startDate, endDate) {
  ...
  var dealRowsFilter_ = function(r){ return r.deal_status === '已成交'; };

  // 行業別反應：本週/累計 來人與成交，直接照 CONFIG.INDUSTRIES 的實際
  // 選項分類，不做任何合併；對不到清單或空白的一律算「未填寫」，不會
  // 無聲消失（見 weeklyReportBucketLabel_ 上方註解）
  function industryBreakdown_(rows) {
    var counts = {};
    WEEKLY_REPORT_INDUSTRY_COLS.concat([WEEKLY_REPORT_UNKNOWN_LABEL]).forEach(function(c){ counts[c] = 0; });
    rows.forEach(function(r) {
      counts[weeklyReportBucketLabel_(r.occupation_industry, WEEKLY_REPORT_INDUSTRY_COLS)]++;
    });
    return counts;
  }
  var industry = {
    ...
    week_deal: industryBreakdown_(weekRows.filter(dealRowsFilter_)),
  ...
  var age = {
    ...
    week_deal: ageBreakdown_(weekRows.filter(dealRowsFilter_)),
    cumulative_deal: ageBreakdown_(cumulativeRows.filter(dealRowsFilter_))
  };

  // ★ v9.36 新增：區域反應（居住行政區），跟行業別/年齡反應同一套邏輯。
  // 居住行政區選項是動態的（Config_Options 表，經理可自行增刪排序，
  // 跟 CONFIG.INDUSTRIES 那種寫死的清單不同），所以每次產生報表時
  // 現抓一次目前的選項，選項改了下次同步就會自動跟著變
  var districtCols = getAllConfigOptions_().district || [];
  function districtBreakdown_(rows) {
    var counts = {};
    districtCols.concat([WEEKLY_REPORT_UNKNOWN_LABEL]).forEach(function(c){ counts[c] = 0; });
    rows.forEach(function(r) {
      counts[weeklyReportBucketLabel_(r.district, districtCols)]++;
    });
    return counts;
  }
  var district = {
    week_visitor: districtBreakdown_(weekRows),
    cumulative_visitor: districtBreakdown_(cumulativeRows),
    week_deal: districtBreakdown_(weekRows.filter(dealRowsFilter_)),
  ...
  // 單選，客戶從路過/591/親友介紹等哪個管道知道案場），跟區域反應同一
  // 套邏輯（單選，不用像戶型反應那樣拆分）。選項是動態的（Config_Options
  // 表，管理者可自行增刪排序），每次產生報表時現抓一次目前的選項
  var sourceCols = getAllConfigOptions_().source || [];
  function sourceBreakdown_(rows) {
    var counts = {};
    sourceCols.concat([WEEKLY_REPORT_UNKNOWN_LABEL]).forEach(function(c){ counts[c] = 0; });
    rows.forEach(function(r) {
      counts[weeklyReportBucketLabel_(r.source, sourceCols)]++;
    });
    return counts;
  }
  var mediaSource = {
    week_visitor: sourceBreakdown_(weekRows),
  ...
  // ★ v9.35 修正：building/unit_type 從 Google Sheets 讀出來時，如果
  // 儲存格內容是純數字（例如型別「1」「2」），Apps Script 會回傳 JS
  // Number 而不是字串。前面拿它們當物件的 key（buildings[u.building]、
  // b.types[u.unit_type]）時，JS 會自動把 key 轉成字串，但後面比對是不是
  // 同一戶時用的是嚴格比較（===），數字 1 跟字串 "1" 永遠不會相等，
  // 導致每一戶都比對不到、整張表格全部空白（連「待售」都不會顯示）。
  // 統一用 String() 包起來比較，不管原始型別是數字還字串都能對得起來
  var buildings = {};
  salesRows.forEach(function(u) {
    var buildingKey = String(u.building);
    if (!buildings[buildingKey]) buildings[buildingKey] = { types: {}, minFloor: Infinity, maxFloor: -Infinity };
    var b = buildings[buildingKey];
    b.types[String(u.unit_type)] = true;
  ...
  var salesControlGrid = Object.keys(buildings).sort().map(function(bName) {
    ...
    var cells = floors.map(function(floor) {
      return types.map(function(type) {
        var unit = salesRows.filter(function(u){
          return String(u.building) === bName && String(u.unit_type) === type && parseInt(u.floor,10) === floor;
        })[0];
  ...
  // 業務統計：依 sales_name 分組。「本週」算的是活動次數（這週接待了
  // 幾組來客、幾組是回籠、幾組成交），「累計」改成算「人數」而不是
  // 「來訪列數」——原本 total_visit/total_revisit 是把初訪＋回籠的列數
  // 全部加在一起，同一人回籠愈多次、回訪率的分子分母會一起膨脹，跟
  // 主管想看的「這些客戶裡有多少比例回來過／成交」意義對不起來（例如
  // 接待 3 組客人、其中 1 組回籠過，回訪率應該是 1/3，不是用來訪列數去
  // 算）。改成先依人（person_id，沒有的話退回 customer_id，跟客戶名單
  // 「一人一張卡」用同一套識別）分組，同一人不管回籠幾次都只算一個人
  var salesNames = {};
  allRows.forEach(function(r){ if (r.sales_name) salesNames[r.sales_name] = true; });
  function weeklyReportPersonKey_(r) { return r.person_id || r.customer_id; }
  var salesStats = Object.keys(salesNames).sort().map(function(name) {
    ...
    mine.forEach(function(r) {
      var key = weeklyReportPersonKey_(r);
      if (!byPerson[key]) byPerson[key] = { revisited: false, deal: false };
      if (r.visit_type === '回籠') byPerson[key].revisited = true;
      if (r.deal_status === '已成交') byPerson[key].deal = true;
    });
    var personKeys = Object.keys(byPerson);
    var totalCustomers = personKeys.length;
    var returningCustomers = personKeys.filter(function(k){ return byPerson[k].revisited; }).length;
    var dealCustomers = personKeys.filter(function(k){ return byPerson[k].deal; }).length;

    return {
      name: name, week_visit: weekVisit, week_revisit: weekRevisit, week_deal: weekDeal,
      total_visit: totalCustomers, total_revisit: returningCustomers, total_deal: dealCustomers,
      revisit_rate: totalCustomers ? Math.round(returningCustomers / totalCustomers * 1000) / 10 : 0,
      deal_rate: totalCustomers ? Math.round(dealCustomers / totalCustomers * 1000) / 10 : 0
    };
  });
...
// v9.39：改用跟 WR_STYLE 一致的配色（{bg,fg} 物件），銷控表格子顏色
// 跟圖例對得上
var SALES_CONTROL_STATUS_COLOR_ = {
  '待售':  { bg: '#EEF5FB', fg: '#17365D' },
  '已保留': { bg: '#FFF8E8', fg: '#9C6500' },
  '已收訂': { bg: '#FCE4B2', fg: '#9C6500' },
  '已簽約': { bg: '#DDEFD8', fg: '#548235' },
  '退戶':  { bg: '#F3F5F7', fg: '#5B6573' }
};
...
function writeWeeklyReportToSheet_(ss, data) {
  ...
  industryRows.forEach(function(pair, i) {
    var counts = pair[1];
    var values = industryColsWithUnknown.map(function(c){ return counts[c] || 0; });
  ...
  ageRows.forEach(function(pair, i) {
    var counts = pair[1];
    var values = ageLabelsWithUnknown.map(function(l){ return counts[l] || 0; });
    var total = values.reduce(function(s,v){ return s+v; }, 0);
    reaction.getRange(11 + i, 2, 1, values.length).setValues([values]);
    reaction.getRange(11 + i, values.length + 2).setValue(total);
  });
  // ★ v9.36 新增：區域反應（居住行政區）
  var districtColsWithUnknown = (data.district_cols || []).concat([WEEKLY_REPORT_UNKNOWN_LABEL]);
  var districtRows = [
    ['本週來人', data.district.week_visitor], ['累計來人', data.district.cumulative_visitor],
    ['本週成交', data.district.week_deal],    ['累計成交', data.district.cumulative_deal]
  ];
  districtRows.forEach(function(pair, i) {
    var counts = pair[1];
    var values = districtColsWithUnknown.map(function(c){ return counts[c] || 0; });
    var total = values.reduce(function(s,v){ return s+v; }, 0);
  ...
  // ★ v9.43 新增：媒體反應（來源管道，單選，不用拆分）
  var mediaSourceColsWithUnknown = (data.media_source_cols || []).concat([WEEKLY_REPORT_UNKNOWN_LABEL]);
  var mediaSourceRows = [
    ['本週來人', data.media_source.week_visitor], ['累計來人', data.media_source.cumulative_visitor],
    ['本週成交', data.media_source.week_deal],    ['累計成交', data.media_source.cumulative_deal]
  ];
  mediaSourceRows.forEach(function(pair, i) {
    var counts = pair[1];
    var values = mediaSourceColsWithUnknown.map(function(c){ return counts[c] || 0; });
    var total = values.reduce(function(s,v){ return s+v; }, 0);
  ...
  // 圖例：跟下面格子的底色一一對應，主管一眼就看得出格子顏色代表哪個
  // 狀態，不用猜
  control.getRange('A2').setValue('圖例');
  wrStyle_(control.getRange('A2'), { bg: WR_STYLE.colHeadBg, fg: WR_STYLE.colHeadFg, bold: true, size: 9, align: 'center' });
  ['待售','已保留','已收訂','已簽約','退戶'].forEach(function(status, i) {
    var cell = control.getRange(2, 2 + i);
    cell.setValue(status);
    var c = SALES_CONTROL_STATUS_COLOR_[status];
    wrStyle_(cell, { bg: c.bg, fg: c.fg, bold: true, size: 9, align: 'center' });
  });
  control.getRange('H2').setValue('狀態以 CRM 最新同步資料為準');
  control.getRange('H2:L2').merge();
  wrStyle_(control.getRange('H2:L2'), { bg: WR_STYLE.zebraB, fg: WR_STYLE.colHeadBg, bold: false, size: 9 });
  ...
  data.sales_control_grid.forEach(function(grp) {
    ...
    wrStyle_(control.getRange(3, col, 1, grp.types.length + 1), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center' });
    grp.floors.forEach(function(floor, r) {
      control.getRange(4 + r, col).setValue(floor + 'F');
      wrStyle_(control.getRange(4 + r, col), { bg: WR_STYLE.zebraB, fg: WR_STYLE.valueFg, bold: true, size: 9 });
      grp.cells[r].forEach(function(cell, c) {
        var rangeCell = control.getRange(4 + r, col + 1 + c);
        if (cell) {
          rangeCell.setValue(cell.status);
          var color = SALES_CONTROL_STATUS_COLOR_[cell.status] || SALES_CONTROL_STATUS_COLOR_['待售'];
          wrStyle_(rangeCell, { bg: color.bg, fg: color.fg, size: 9, align: 'center' });
        }
      });
    });
  ...
  var nameHeaderRange = salesSheet.getRange(3, 3, 1, names.length + 1);
  nameHeaderRange.setValues([names.concat(['總計'])]);
  wrStyle_(salesSheet.getRange(3, 3, 1, names.length), { bg: WR_STYLE.sectionBg, fg: WR_STYLE.sectionFg, bold: true, size: 10, align: 'center' });
  wrStyle_(salesSheet.getRange(3, 3 + names.length), { bg: WR_STYLE.totalColBg, fg: WR_STYLE.valueFg, bold: true, size: 9, align: 'center' });
  var metrics = ['week_visit','week_revisit','week_deal','total_visit','total_revisit','total_deal',null,null];
  var salesRowBg = [WR_STYLE.weekRowBg, WR_STYLE.weekRowBg, WR_STYLE.weekRowBg,
    WR_STYLE.cumRowBg, WR_STYLE.cumRowBg, WR_STYLE.cumRowBg, WR_STYLE.effBg, WR_STYLE.effBg];
  for (var i = 0; i < 8; i++) {
    var row = 4 + i;
    var vals3;
    if (i === 6) vals3 = data.sales_stats.map(function(s){ return s.revisit_rate + '%'; });       // 回訪率
    else if (i === 7) vals3 = data.sales_stats.map(function(s){ return s.deal_rate + '%'; });      // 成交比
    else vals3 = data.sales_stats.map(function(s){ return s[metrics[i]]; });
    salesSheet.getRange(row, 3, 1, vals3.length).setValues([vals3]);
    wrStyle_(salesSheet.getRange(row, 3, 1, vals3.length), { bg: salesRowBg[i], fg: WR_STYLE.valueFg, size: 9, align: 'center' });
    if (i < 6) {
      var total = vals3.reduce(function(s,v){ return s+v; }, 0);
      salesSheet.getRange(row, 3 + vals3.length).setValue(total);
      wrStyle_(salesSheet.getRange(row, 3 + vals3.length), { bg: WR_STYLE.totalColBg, fg: WR_STYLE.valueFg, bold: true, size: 9, align: 'center' });
    }
  }
  ...
  settingsRows.forEach(function(rowVals, i) {
    var r = 2 + i;
    var isLast = i === settingsRows.length - 1;
    var bg = isLast ? WR_STYLE.totalBg : (i % 2 === 0 ? WR_STYLE.zebraA : WR_STYLE.zebraB);
    var fg = isLast ? WR_STYLE.totalFg : WR_STYLE.valueFg;
    wrStyle_(settings.getRange(r, 1, 1, 4), { bg: bg, fg: fg, bold: isLast, size: 9 });
    wrStyle_(settings.getRange(r, 1), { bold: true });
  });
}
