 ...
 * ============================================================
 *  龍登 CRM — 吉隆天曜專用版 v1.0
 *  全新案場，直接以「華雄音樂匯 v9.3」完整版為基礎建立，從第一天
 *  上線就含完整功能（不用像 hstd/hsyy 一樣一路補齊）：
 *  - 每日日報：防止同一人同一天重複提交、可刪除日報
 *  - 客戶／任務／維修：都可以刪除（業務只能刪自己建立的，
 *    主管/admin 可以刪任何一筆）
 *  - 已成交客戶可標記「退戶」（可附原因），寫入
 *    Customer_Change_Log 留下完整異動紀錄
 *  - 電話號碼、日期時間欄位皆強制存成文字，避免 Google Sheets
 *    自動轉型造成開頭 0 遺失或時區位移
 *  - 銷售日報歷史／週比較、排班頁面月曆重要事項、下週休假通報
 *    推播等功能皆已內建
 *  - 不含 hstd 專屬的「平日休假 2 人上限」限制規則（如果吉隆天曜
 *    也需要類似限制，請再告知）
 * ============================================================
 *  首次部署：
 *  1. 試算表 → 擴充功能 → Apps Script → 貼入此檔全部內容
 *     （CONFIG.SPREADSHEET_ID 已經預先填好這個案場的試算表 ID，
 *     不用再改）
 *  2. 執行 firstTimeSetup()（預設密碼 075500888，建議上線前更換）
 *  3. 部署 → 新增部署 → Web App（執行身分=我, 存取=任何人）
 *  4. 複製 /exec 網址 → 貼到 jltx.html 的 GAS_URL
 *  5. 上傳 jltx.html 到 GitHub Pages／Cloudflare
 *  6. LINE Developers Console 建立 LIFF App，Endpoint URL 填
 *     jltx.html 的網址，取得 LIFF ID → 貼到 jltx.html 的 LIFF_ID
 *  7.（要推播才需要）執行 setLineToken(...)／setLinePushTarget(...)／
 *     setLineChannelSecret(...)，填這個案場自己 LINE 官方帳號的憑證
 *  8. 執行 addMyself() 或 addUser('你的userId','你的名字','admin',
 *     '吉隆天曜') 加入第一位管理員
 * ============================================================
 */
...
function appendDailyReport(payload) {
  try {
    ...
    appendObjectToSheet(CONFIG.SHEETS.DAILY_REPORT, {
      report_id: reportId, report_date: reportDate,
...
/* 吉隆天曜是全新案場，需要自己獨立的 LINE 官方帳號 Token／Channel Secret，
   不能沿用華雄音樂匯的憑證。等你設定好吉隆天曜自己的 LINE 官方帳號後，
   把下面兩個函式的參數換成該帳號的 Token／Channel Secret 再執行。 */
function setupLine() {
  setLineToken('請貼上吉隆天曜LINE官方帳號的_CHANNEL_ACCESS_TOKEN');
  setLinePushTarget('U4bf4bf6035e402e4d5a17a01915812bc');
}
function setupSecret() { setLineChannelSecret('請貼上吉隆天曜LINE官方帳號的_CHANNEL_SECRET'); }
function addMyself() {
