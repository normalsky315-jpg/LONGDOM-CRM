...
## 追加項目九：週報表新增「戶型反應」「媒體反應」

使用者希望週報表能看出客戶對哪些戶型比較感興趣，以及是從哪個媒體/管道知道案場的，跟既有的行業別反應/年齡反應/區域反應同一套邏輯，各自新增一張表。

**戶型反應**：資料來源是客戶登記表單「需求房型」（`room_types`），這是複選框（2房/3房/4房/店面），一位客戶可能同時勾好幾個。使用者明確要求：勾了 N 個已知戶型，每個戶型各分到 `1/N`（例如勾兩個戶型各算 0.5，勾三個各算 0.33），不能每個都算一次（否則一組客戶會被灌成好幾組來人），確保「加總永遠等於實際來人數/成交數」這個既有的不變量繼續成立。完全沒勾到已知戶型（含空白）才整組算 1 歸進「未填寫」。數字四捨五入到小數點後兩位方便閱讀。

**媒體反應**：資料來源是客戶登記表單「來源管道」（`source`，單選，路過/591/親友介紹等），跟區域反應完全同一套邏輯（單選不用拆分，選項是 `Config_Options` 動態設定，每次同步現抓一次）。

**版面**：接在區域反應（row 16 起）下面，戶型反應 row 23 起、媒體反應 row 30 起。`syncWeeklyReportLayout_` 本來就設計成每次同步都會重新檢查並修正版面（v9.36 就是這樣把區域反應加進舊試算表的），所以使用者現有的週報試算表下次按「同步本週報表」就會自動補上這兩張新表，不需要額外的搬遷步驟或重建試算表。
